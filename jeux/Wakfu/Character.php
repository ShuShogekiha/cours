...
class Character
{
     ...
     */
    public function __construct(string $nom, int $hp = 15, int $dps = 5, int $move = 3, int $action = 4, int $armure = 0, string $tech = "aucun skill", string $equip = "aucun equipement")
    {
        $this->name = $nom;
        $this->pv = $hp;
        $this->damage = $dps;
    ...
    {
        return "<p>Nom : " . $this->getName() . "</p><p>PV : " . $this->getPv() . "</p><p>Degats = " . $this->getDamage() . "</p><p>PA : " . $this->getPa() . "</p><p>PM : " . $this->getPm() . "</p><p>Armure : " . $this->getArmor() . "</p><p>Skill : " . $this->getSkill() . "</p><p>Stuff : " . $this->getStuff() . "</p>";
    }
    ...
    public static function attaque($attaquant, $cible)
    {
        if ($cible->getPv() >= 0) {
            $cible->setPv($cible->getPv() - $attaquant->getDamage());
            if ($cible->getPv() <= 0) {
                $cible->setP(0);
            }
        } else {
            echo "deja mort, il subis 0";
        }
    }
    public static function soin($cible)
    {
        if ($cible->getPv() < 15) {
            $cible->setPv($cible->getPv() + 3);
            if ($cible->getPv() >= 15) {
                $cible->setPv(15);
            }
        } else {
            echo "il est full vie ";
        }
    }
}
?>